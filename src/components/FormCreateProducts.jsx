...
function FormCreateProducts() {
  ...
  const dropImage = async (image) => {
    ...
    try {
      const res = await axiosInstance
        .delete(`/api/admin/image/${image._id}`);
      ...
      else { throw new Error(`[${res.status}] ERROR en la solicitud`) };
    } catch (error) {
  ...
  const getValues = async () => {
    ...
    try {
      const res = await axiosInstance
        .get(`/api/admin/products/${params.id}`)
          ...
          if (typeof (params.id) != "undefined") {
            try {
              res = await axiosInstance
                .put(`/api/admin/products/${params.id}`, data, {
                  headers: {
      ...
      >
        {({ values, isSubmitting, errors, touched }) => (
          <Form className="flex flex-col p-4 md:px-8 max-w-5xl m-auto">
                    ...
                    {images.map((image) => (
                      ...
                      <li key={image._id} className="p-4">
                        <div className="relative size-24">
                          <img className="size-full mr-3" alt="Imagen de WhatsApp 2025-03-20 a las 15.15.09_d5242c1d.jpg" src={`${API_URL}/public/images/products/${image.url}`}></img>
                          <button type="button" className="bg-transparent absolute top-0 right-0" onClick={() => dropImage(image)}>
            ...
            <Dropzone files={files} setFiles={setFiles} className="p-16 mt-10 border border-neutral-200 text-center mb-3" />
            ...
            <Field
              ...
              name="category"
            >
              {category.map((cat) => (<option title={cat.description} key={cat._id} value={cat._id}>{cat.name}</option>))}
            </Field>
